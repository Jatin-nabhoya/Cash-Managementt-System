...
# 			cur.execute("update user set docter ='" + docter_var.get() + "', day ='" +  day.get() + "', month = '" + month.get() + "', year = '" + year.get() + "' where username ='"+ user_name.get() +"'")
# 			con.commit()	
# 			con.close()
	...
	def action():
		if first_name.get()=="" or last_name.get()=="" or age.get()=="" or city.get()=="" or add.get()=="" or user_name.get()=="" or password.get()=="" or very_pass.get()=="":
			...
			try:
				con = sqlite3.connect("data.db")
				cur = con.cursor()
				cur.execute(f"select * from user where username={user_name.get()}")
				row = cur.fetchone()
				if row!=None:
					messagebox.showerror("Error" , "User Name Already Exits", parent = winsignup)
				else:
					cur.execute("insert into user(first_name,last_name,age,gender,city,address,username,password) values(?,?,?,?,?,?,?,?)",
						(
						first_name.get(),
						last_name.get(),
						age.get(),
